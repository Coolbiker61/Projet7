...
const listenerEvent = () => {
    ...
    document.getElementById('title').addEventListener('input', function () {
        ...
        for (let part of title) {
            if (/(['\|\/\\\*\+&#"\{\(\[\]\}\)$£€%=\^`])/g.test(part) && !(/(^([\wáàâäãåçéèêëíìîïñóòôöõúùûüýÿæœÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ]{1})([\w'\sáàâäãåçéèêëíìîïñóòôöõúùûüýÿæœÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ]{1,})$)/gi.test(part))) {
                error = true;
            }
        }
